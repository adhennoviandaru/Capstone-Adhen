...
def load_data():
    # Read data
    insurance = pd.read_csv(category)
...
def plot_incident(data):
    ...
    timeseries = insurance.pivot_table(
                index='incident_date',
...
def plot_baru(data):
    ...
    own_analysis = insurance[insurance['incident_severity'] =='Total Loss']
    pd.crosstab(index=own_analysis['fraud_reported'],
    columns= ('percentage'),
    normalize=True).round(4)*100

    # ---- Incident severity

    insurance_fraud = insurance[insurance['fraud_reported']=='Y']
    percentage_fraud=pd.crosstab(index=insurance_fraud['incident_severity'],
    columns= 'fraud')
    percentage_total=pd.crosstab(index=insurance['incident_severity'],
    columns= 'total')    
    ax = percentage_total.plot.bar(title='Perbandingan Kasus dan Fraud',xlabel='Incident Severity',ylabel='Jumlah Kasus')
